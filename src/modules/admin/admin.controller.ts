...
export class AdminController {
    ...
    @Get('state')
    getStates(): { models: ModelState[]; timestamp: string } {
        return {
    ...
    @Get('state/:modelName')
    getState(@Param('modelName') modelName: string): ModelState {
    ...
    resetState(@Param('modelName') modelName: string): { message: string } {
    ...
    getMetrics() {
        ...
        const modelsAvailable = states.filter(s => s.circuitState === 'CLOSED' || s.circuitState === 'HALF_OPEN').length;
        const modelsInOpenState = states.filter(s => s.circuitState === 'OPEN').length;
        const modelsPermanentlyUnavailable = states.filter(s => s.circuitState === 'PERMANENTLY_UNAVAILABLE').length;
