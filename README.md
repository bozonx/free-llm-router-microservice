...
#### Система тегов (новая логика фильтрации)

Скрипт `update-models` автоматически проставляет теги для моделей на основе их характеристик. Система фильтрации в роутере использует логику **DNF (Дизъюнктивная нормальная форма)**:
...
- `tags: "llama, qwen"` — найдет любую модель Llama **ИЛИ** любую модель Qwen (в старой версии это означало бы поиск гибрида Llama AND Qwen).
- `tags: "coding&tier-1, reasoning&tier-2"` — найдет либо топовую модель для кодинга, либо хорошую модель для рассуждений.
- `tags: ["best-for-ru", "best-for-en"]` — найдет модель, подходящую либо для русского, либо для английского.
...
- Для логики **AND** (И) внутри одного правила используйте символ `&`.
- Старая логика `|` внутри тега (например, `medium|large`) всё еще поддерживается для обратной совместимости, но рекомендуется использовать основной синтаксис.
...
# Найти ЛИБО топовую модель для кодинга, ЛИБО любую модель Llama
curl -X POST http://localhost:8080/api/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "tags": "coding&tier-1, llama",
    "messages": [...]
  }'
```
