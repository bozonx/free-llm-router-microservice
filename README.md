...
### Выбор модели (Smart Strategy)

1. Если указан конкретный `model` — используем его
2. Фильтруем модели по критериям (`tags`, `type`, `min_context_size`, `json_response`)
3. Исключаем модели с открытым Circuit Breaker (OPEN, PERMANENTLY_UNAVAILABLE)
4. Исключаем модели, превысившие `maxConcurrent` лимит
5. Если указан `min_success_rate` — исключаем модели с низким success rate
6. Если `prefer_fast: true` — выбираем модель с наименьшей latency
7. Группируем модели по приоритету (`priority`)
8. В группе с наивысшим приоритетом выполняем weighted random selection по `weight`
