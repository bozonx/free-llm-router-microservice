...
- 🔄 **Автоматический выбор модели** — Smart Strategy с приоритетами и весами
...
3. **Smart Strategy** — если `model: "auto"` или не указан:
   ...
   - Группируем модели по приоритету (`priority`)
   - В группе с наивысшим приоритетом выполняем weighted random selection по `weight`


### Обработка ошибок и fallback

1. При ошибке **429** (Rate Limit) — ретрай до `rateLimitRetries` раз с задержкой + jitter (на той же модели)
2. При ошибках **4xx** (кроме 429) — прервать выполнение, вернуть ошибку клиенту
3. При ошибках **5xx, timeout, сетевых ошибках** — переключиться на следующую модель
4. Повторяем до `maxRetries` попыток (переключение между моделями)
5. Если все бесплатные модели исчерпаны и `fallback.enabled = true` — используем платную модель
6. Возвращаем результат с полной информацией о попытках в поле `_router`

### Обработка различных кодов ошибок

| Код ошибки | Действие | Circuit Breaker | Прерывает цикл? |
|------------|----------|------------------|-----------------|
| 404 (Not Found) | Модель не существует, прервать | `PERMANENTLY_UNAVAILABLE` — исключение до рестарта | ✅ Да |
...
**Примечания:**
- **Ретрай** — повторная попытка на той же модели с задержкой
- **Переключение** — немедленный переход к следующей модели без задержки
- **Circuit Breaker** учитывает все ошибки (включая 4xx) в статистике для расчета success rate
- Только 404 переводит модель в состояние `PERMANENTLY_UNAVAILABLE`
- Клиентские ошибки (4xx кроме 429) прерывают весь цикл попыток и возвращают ошибку клиенту
...
**Дизайн:**
- Modern dark mode с glassmorphism эффектами
- Автообновление каждые 5 секунд
- Responsive дизайн для mobile и desktop
- Цветовая кодировка статусов
- Плавные анимации и микроinteractions

См. подробности в [`public/README.md`](./public/README.md)
