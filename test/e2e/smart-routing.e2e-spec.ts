...
describe('Smart Routing (e2e)', () => {
    ...
    describe('Circuit Breaker', () => {
        ...
        it('should open circuit after failures and skip model', async () => {
            ...
            let adminResponse = await app.inject({
                method: 'GET',
                url: '/api/v1/admin/state'
            });
            let state = JSON.parse(adminResponse.body);
            let modelState = state.models.find((m: any) => m.name === targetModel);
            ...
            const openRouterScope2 = nock('https://openrouter.ai')
                .post('/api/v1/chat/completions', () => true)
                .reply(500, { error: 'Should not happen', usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } });
        ...
        it('should mark model as PERMANENTLY_UNAVAILABLE on 404', async () => {
            ...
            nock('https://openrouter.ai')
                .post('/api/v1/chat/completions', () => true)
                .reply(404, { error: { message: 'Model not found' }, usage: { prompt_tokens: 0, completion_tokens: 0, total_tokens: 0 } });

            // Mock Fallback
            nock('https://api.deepseek.com')
                .post('/chat/completions', () => true)
                .reply(200, { choices: [{ message: { content: 'ok' } }], usage: { prompt_tokens: 10, completion_tokens: 10, total_tokens: 20 } });
