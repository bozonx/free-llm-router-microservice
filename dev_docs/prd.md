...
# Новые
CONFIG_PATH=./config/router.yaml  # Путь к конфигу роутера
```
...
| Код ошибки | Действие |
|------------|----------|
| 429 (Rate Limit) | Retry с другой моделью |
...
### v1.3
- [ ] Webhook для уведомлений о проблемах с моделями
...
- нужен параметр задержки между попытками, так же будет jitter - 20% +- само значение проццентов захардкомжено в глоальной констенте
- при ошибках 429 нужно сделать несколько повторов и только потом переходит к следующей модели
- Webhook тогда не делаем, в будущем будет otel
