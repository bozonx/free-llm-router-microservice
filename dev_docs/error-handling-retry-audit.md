...
2. При **сетевых ошибках** (ENETUNREACH, ECONNRESET) — ретрай до `maxSameModelRetries` раз (проблема может быть временной)
...
6. Повторяем до `maxModelSwitches` попыток (переключение между моделями)
